...
class WhatsAppBot {
    ...
    async handleMessage(message) {
        try {
            ...
            let commandName = args.shift()?.toLowerCase() || ''; // Added safe navigation and default to empty string

            // --- FIX START: Explicitly handle empty command name (i.e., when only prefix '!' is sent) ---
            if (commandName === '') {
                // If only prefix is sent, treat it as calling the 'help' command
                commandName = 'help';
                // Clear args since the 'help' command itself was not explicit
                args.length = 0; 
                
                // If the help command is not loaded, still reply with command not found.
                if (!this.commands.has('help')) {
                    if (!this.config.hideNotiMessage.commandNotFound) {
                        // Use original prefix for reply, but state the missing command was 'help'
                        await messageWrapper.reply(`>🎀\n𝐓𝐡𝐞 𝐜𝐨𝐦𝐦𝐚𝐧𝐝 "help" 𝐝𝐨𝐞𝐬 𝐧𝐨𝐭 𝐞𝐱𝐢𝐬𝐭, 𝐭𝐲𝐩𝐞 ${prefix}𝐡𝐞𝐥𝐩 𝐭𝐨 𝐬𝐞𝐞 𝐚𝐲𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬`);
                    }
                    return;
                }
            }
